...
public class SIRGroupModel extends AbstractGroupModel<SIRGroup> {
	...
	private void initializeGroupsOfInitialPedestrians() {
		...
		if (c.getElements().size() > 0) {
			// assign Pedestrians in the scenario at the beginning to different groups.
	...
	public void update(final double simTimeInSec) {
		...
		DynamicElementContainer<Pedestrian> c = topography.getPedestrianDynamicElements();
		LinkedCellsGrid<Pedestrian> cell_grid = topography.getSpatialMap(Pedestrian.class);

		// Check if there are any pedestrians in the simulation.
		if (c.getElements().size() > 0) {
			// Loop through all pedestrians in the simulation and update their group.
			for (Pedestrian p : c.getElements()) {
				// If the pedestrian is infected, check if they recover based on the recovery rate attribute.
				...
				// If the pedestrian is susceptible, check if they get infected based on their distance to infected pedestrians and the infection rate attribute.
				else if (getGroup(p).getID() == SIRType.ID_SUSCEPTIBLE.ordinal()) {
					// Get all neighboring pedestrians within the infection max distance of the current pedestrian.
					List<Pedestrian> real_neighbors = cell_grid.getObjects(p.getPosition(), attributesSIRG.getInfectionMaxDistance());
					// loop over neighbors and set infected if we are close
					for (Pedestrian p_neighbor : real_neighbors) {
